...
func (t *RollbackTool) Execute(ctx context.Context, args map[string]interface{}, exec executor.Executor) (string, error) {
	...
	case int64:
		cpID = v
	default:
		return "", fmt.Errorf("checkpoint_id must be an integer")
	}
	...
	if cmd == "" {
		return "이 체크포인트에는 저장된 롤백 명령이 없습니다.", nil
	}
	...
	return fmt.Sprintf("✓ 롤백 성공 (CP #%d)\n명령: %s\n출력:\n%s", cpID, cmd, res.Stdout), nil
}
