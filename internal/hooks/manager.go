// Package hooks
// File: manager.go
// Description: 훅 생명주기 관리자 — 등록, 발화, 활성화/비활성화, 삭제
...
func (m *Manager) Register(ctx context.Context, event Event, condition, scriptPath string) (int64, error) {
	...
	slog.Info("hook registered", "id", id, "event", event, "script", scriptPath)
	return id, nil
}
...
func (m *Manager) Fire(ctx context.Context, hctx HookContext) {
	...
	for _, h := range hooks {
		...
		if err := m.runScript(ctx, h, hctx); err != nil {
			slog.Warn("hook script failed", "id", h.ID, "script", h.ScriptPath, "err", err)
		}
	}
}
...
func (m *Manager) runScript(ctx context.Context, h store.Hook, hctx HookContext) error {
	...
	slog.Debug("hook executed", "id", h.ID, "event", hctx.Event, "output", strings.TrimSpace(string(out)))
	return nil
}
