...
func (t *RegisterTool) Execute(ctx context.Context, args map[string]interface{}, _ executor.Executor) (string, error) {
	...
	if event == "" || scriptPath == "" {
		return "", fmt.Errorf("event와 script_path는 필수입니다")
	}

	id, err := t.Manager.Register(ctx, Event(event), condition, scriptPath)
	if err != nil {
		return "", fmt.Errorf("훅 등록: %w", err)
	}
	...
	return fmt.Sprintf("✓ 훅 #%d 등록 완료\n이벤트: %s\n스크립트: %s\n조건: %s",
		id, event, scriptPath, condDisplay), nil
}
